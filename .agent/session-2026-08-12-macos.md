...
## v3.15~v3.20 수동 테스트 가이드 작성 (문서 보강)

### 1. 무엇을
- `docs/tests/`에 기존 v3.10~3.14만 존재하던 갭을 채움 — **v3.15~v3.20 가이드 6개 신규 작성** (기존 형식/톤 유지):
  - `v3.15_macos.md`: 커스텀 배경(업로드/제거/영속성) + 카드 뒷면 5종 + 카드 앞면 4종
  - `v3.16_macos.md`: 데일리 딜(진입/변형별 분리/확인 다이얼로그/통계)
  - `v3.17_macos.md`: 데일리 챌린지(별점 3)/업적 10종/승리 시 동시 반영
  - `v3.18_macos.md`: Scorpion(그룹 이동/예비 더미/빈 열 K/승리 4열/저장)
  - `v3.19_macos.md`: 점수 체계(이동 누적/승리 보너스/undo 롤백/통계/변형별)
  - `v3.20_macos.md`: Winnable 옵션(ON/OFF/지연/4종 확인/영속성)
...
### 4. 남은 TODO
- [ ] 수동 플레이 검증 (v3.15~v3.20 — 가이드가 준비됐으니 이제 가이드대로 수행 가능)
- [ ] T-206: #50/#500 기본 예산 내 해결 — 보류

### 5. 다음 에이전트 전달 로그
- 커밋 예정: `docs(macos): v3.15~v3.20 수동 테스트 가이드 작성`
- 수동 검증 시 가이드는 docs/tests/v3.{15..20}_macos.md 순서로 수행하면 됨.
- 에러코드 없음.

### 6. 문서 업데이트 목록
- docs/tests/v3.15~v3.20_macos.md (신규 6개), 세션 로그
