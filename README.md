...
| **Pyramid** | 피라미드 28장, 합 13인 노출 카드 제거, 스톡 재활용 1회 |
...
- **게임 모드**: 설정에서 일반(4판: FreeCell·Klondike·Spider·Pyramid) / 확장(12종) 전환 — 홈·번호 시트·무작위·오늘 딜이 모드 내 변형만 표시, 진행 중 게임은 모드 전환 후에도 유지
- **데일리 도전**: 모드별 판 수(일반 4판/확장 9판) 날짜 시드 셔플 · 3개월 달력(날짜별 ★완료) · 별점(승리·시간·이동) · 월 통계·월 배지(브론즈/실버/골드/다이아) · 판별 난이도 태그
...
- 단위 테스트 **274개** 통과 (딜 재현 · 이동 규칙 · 수퍼무브 용량 · 승리 판정 · 저장/복원 · 데일리 도전/챌린지/업적 · Winnable 솔버 · 자동 풀어 보기 · 난이도 판정 · 카테고리/난이도 표시 · 모드 분리 · Pyramid 재활용)
